...
public class Simulation {
	...
	void blockTramAtTramStop() {
		...
		for (int i = 0; i < trafficNetwork.vehicles.size(); i++) {
			...
			if (!vehicle.active || (vehicle.lane == null) || !vehicle.workerName.equals(trafficNetwork.ownerName)) {
				continue;
			}
		...
		for (final Edge edge : edges) {
			for (final Lane lane : edge.lanes) {
				for (final Vehicle vehicle : lane.vehicles) {
					...
					// Trajetories collected (for vehicle) in workers and passed between them as vehicle parameter
					if (Settings.collectTrajectoryInWorkers) {
						final double[] coordinates = VehicleUtil.calculateCoordinates(vehicle);
						vehicle.trajectories.add(new VehicleTrajectory(timeNow, coordinates[1], coordinates[0]));
//				vehicle.addVehicleTrajectory(timeNow, vehicle);
					}
	...
	void moveVehicleToNextLink(final double timeNow, final ArrayList<Vehicle> vehiclesToCheck) {
		for (final Vehicle vehicle : vehiclesToCheck) {
			...
			if (!vehicle.active || !vehicle.workerName.equals(trafficNetwork.ownerName)) {
				continue;
			}
...
//                System.out.printf("Move vehicle with this id: %s and route index: %d\n", vehicle.id, vehicle.indexLegOnRoute);
	...
	void pause() {
		if (Settings.pauseTimeBetweenStepsInMilliseconds > 0) {
			try {
				...
				e.printStackTrace();
				logger.error("Simulation.pause() ", e);
			}
		}
	}
	...
	boolean reachFellow(final Vehicle vehicle) {
		...
		for (final Fellow fellowWorker : connectedFellows) {
			for (final Edge edge : fellowWorker.inwardEdgesAcrossBorder) {
				if (edge.index == vehicle.lane.edge.index) {
	...
	synchronized void simulateOneStep(final Worker worker, boolean isNewNonPubVehiclesAllowed,
		...
		if(worker.numLocalRandomBuses !=0 || worker.numLocalRandomTrams != 0 || !Settings.oneSimulation){
            trafficNetwork.createInternalVehicles(worker.numLocalRandomPrivateVehicles, worker.numLocalRandomTrams,  //TODO change it!
            ...
            if(WorkerManager.useServiceToCreateVehicles){
                worker.finishVehicles = oneStepData_allVehiclesReachedDestination.size();
            } else{
                trafficNetwork.createInternalNonPublicVehicles(oneStepData_allVehiclesReachedDestination.size(), worker.timeNow);
            }
