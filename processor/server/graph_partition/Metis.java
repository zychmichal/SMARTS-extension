...
public class Metis {
    //public final static RegularFile CMD = new RegularFile(new Directory("/usr/local/bin/"), "gpmetis.exe");
    ...
    public List<IntSet> compute(Graph<Integer, DefaultWeightedEdge> g, Map<Integer, GridCell> indexCellGraph, int numberOfPartitions, Random r, Map<String, GridCell> workCellsSet)
    {
        return compute(g,indexCellGraph, numberOfPartitions, Ptype.kway, Ctype.shem, Iptype.random, Objtype.vol, false, false, 30, 10, 1, r, workCellsSet);
    }

    public List<IntSet> compute(Graph<Integer, DefaultWeightedEdge> g, Map<Integer, GridCell> indexCellGraph, int numberOfPartitions, Ptype ptype, Ctype ctype, Iptype iptype, Objtype objtype, boolean contig, boolean minconn,
                                int ufactor, int niter, int ncuts, Random r, Map<String, GridCell> workCellsSet)
    {
        ...
        RegularFile inFile = new RegularFile(Directory.getCurrentDirectory(), "metis-input-file-" + numberOfPartitions);
        OutputStream os = inFile.createWritingStream();
        new MetisWriter().printGraph(g,indexCellGraph, workCellsSet, os);
...
//         Proces.TRACE_CALLS = true;
        ...
        parms.addAll(Arrays.asList("-ufactor=" + ufactor, "-niter=" + niter, "-ncuts=" + ncuts, "-seed=" + r.nextInt(), inFile.getName(), ""
                + numberOfPartitions));
        byte[] stdout = Proces.exec("gpmetis", inFile.getParent(), parms.toArray(new String[0]));

        inFile.delete();
        System.out.println(new String(stdout));

        // retrieves the content from the file generated by METIS
        //TUTAJ MOZNA METIS FILA PO PROSTU OTWORZYC Z WCZESNIEJ WYGENEROWANEGO
