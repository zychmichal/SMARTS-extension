...
class MetisWriter {
    ...
    String printGraph(Graph<Integer, DefaultWeightedEdge> g, Map<Integer, GridCell> indexCellGraph, Map<String, GridCell> workCellsSet, OutputStream ss) {
...
//            throw new IllegalArgumentException("graph has vertex 0 which is not supported by metis");
        for(int vertex:g.vertexSet()){
            ...
            if(Objects.requireNonNull(indexCellGraph.get(vertex)).laneLength < 1)
                os.print(1);
            else
                os.print(Objects.requireNonNull(indexCellGraph.get(vertex)).laneLength);
