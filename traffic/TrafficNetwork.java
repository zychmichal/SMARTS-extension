...
public class TrafficNetwork {
	...
	public ArrayList<Edge> internalNonPublicVehicleStartEdges = new ArrayList<>();
	public ArrayList<Edge> internalNonPublicVehicleEndEdges = new ArrayList<>();
	...
	public String internalVehiclePrefix = "";
	 ...
	 */
	public TrafficNetwork(String ownerName, RoadNetwork roadNetwork, Worker worker) {
	    if(roadNetwork == null){
    ...
    public TrafficNetwork(RoadNetwork roadNetwork){
        this.roadNetwork = roadNetwork;
    }
	...
	public Vehicle createVehicle(final VehicleType type, final boolean isExternal, final boolean foreground,
        ...
        worker.changeVehicleRoutesEdgesInWorker(vehicle);
	...
	public void addNewVehicle(final VehicleType type, final boolean isExternal, final boolean foreground,
					   final ArrayList<RouteLeg> routeLegs, final String idPrefix, final double timeRouteStart,
					   final String externalId, final DriverProfile dP) {
		...
		Vehicle vehicle = createVehicle(type, isExternal, foreground, routeLegs, idPrefix, timeRouteStart, externalId, dP);
	...
	void addTramStopsToParallelNonTramEdges() {
		...
		for (final Edge tramEdge : internalTramStopEdges) {
			candidateEdges = roadNetwork.getEdgesParallelToEdge(tramEdge.startNode.lat, tramEdge.startNode.lon,
    ...
    public ArrayList<RouteLeg> createRoute(VehicleType type){
        int counter = 0;
        ArrayList<RouteLeg> route = null;
        while(route == null && counter < 10){
            route = createOneRandomInternalRoute(type);
            counter++;
        }
//        if(WorkerManager.printStream && counter>1) System.out.println("GENEROWANO ROUTY WIECEJ NIZ RAZ, A DOKLADNIE -> " + counter);
        return route;
    }


    public ArrayList<RouteLeg> createRouteToService(VehicleType type){
//        if(WorkerManager.printStream && counter>1) System.out.println("GENEROWANO ROUTY WIECEJ NIZ RAZ, A DOKLADNIE -> " + counter);
        return createOneRandomInternalRoute(type);
    }

    /**
     * Create non-public vehicle -> use parameter as as the number of vehicles to create
     */
    public void createInternalNonPublicVehicles(int numberVehiclesToCreate, final double timeNow){
        long printCreatingStatusTimeStamp = System.nanoTime();
        for (int i = 0; i < numberVehiclesToCreate; i++) {
            VehicleType type = chooseType();
            ArrayList<RouteLeg> route;
            route = createRoute(type);
            if(route == null){
                System.out.println("AFTER 10 TIMES DONT CREATE A VEHICLE");
            } else{
            ...
            if(i%100==0 && i>0){
                System.out.println("Worker " + worker.name + " create: " + i + " vehicles, time = " + (System.nanoTime()-printCreatingStatusTimeStamp)/1.0e9);
                printCreatingStatusTimeStamp = System.nanoTime();
            }
        }
    }
    ...
    public void createInternalNonPublicVehicles(int numLocalRandomPrivateVehicles, final double timeNow,
		...
		if (isNewNonPubVehiclesAllowed) {
			...
			createInternalNonPublicVehicles(numVehiclesNeeded, timeNow);
		}
	}
    ...
    public void createInternalReservedVehiclesToService(int numVehicles, ArrayList<Vehicle> reservedVehicles){
        for (int i=0; i< numVehicles; i++){
            ...
            if (route != null) {
                Vehicle vehicle = createVehicle(type, false, false, route, internalVehiclePrefix, -1, "", getRandomDriverProfile());
...
//	 * @param strategy
	...
	ArrayList<RouteLeg> createOneRouteFromSerializedData(final ArrayList<SerializableRouteLeg> serializedData) {
		final ArrayList<RouteLeg> route = new ArrayList<>(100);
		for (final SerializableRouteLeg sLeg : serializedData) {
			final RouteLeg leg = new RouteLeg(roadNetwork.edges.get(sLeg.edgeIndex), sLeg.stopover);
			route.add(leg);
		}
		return route;
	}

	ArrayList<VehicleTrajectory> createOneVehicleTrajectoryFromSerializedData(final ArrayList<SerializableVehicleTrajectory> serializableVehicleTrajectories) {
		final ArrayList<VehicleTrajectory> trajectoryArrayList = new ArrayList<>(100);
		for (final SerializableVehicleTrajectory sTrajecotry : serializableVehicleTrajectories) {
			final VehicleTrajectory trajectory = new VehicleTrajectory(sTrajecotry.timeStamp, sTrajecotry.lat, sTrajecotry.lon);
			trajectoryArrayList.add(trajectory);
		}
		return trajectoryArrayList;
	}

	public DriverProfile getRandomDriverProfile() {
    ...
    public DriverProfile getRandomDriverProfile(Random random) {
        final double r = random.nextDouble();
        for (int i = 0; i < DriverProfile.values().length; i++) {
            if (r < driverProfilePercAccumulated.get(i)) {
                return DriverProfile.values()[i];
	 ...
	 */
	//HERE ONLY WORKAREA SHOULD BE ENOUGH?
	...
	public void identifyInternalVehicleRouteStartEndEdges() {
		...
		for (final GridCell cell : workareaCells) {
			for (final Node node : cell.nodes) {
				if (((Settings.listRouteSourceWindowForInternalVehicle.size() == 0)
						...
						&& roadNetwork.isNodeInsideRectangle(node, Settings.listRouteSourceWindowForInternalVehicle))
						|| ((Settings.listRouteSourceDestinationWindowForInternalVehicle.size() > 0)
						&& roadNetwork.isNodeInsideRectangle(node,
		...
		for (final Edge edge : roadNetwork.edges) {
			if (((Settings.listRouteDestinationWindowForInternalVehicle.size() == 0)
					&& (Settings.listRouteSourceDestinationWindowForInternalVehicle.size() == 0))
					|| ((Settings.listRouteDestinationWindowForInternalVehicle.size() > 0) && roadNetwork.isNodeInsideRectangle(
					edge.endNode, Settings.listRouteDestinationWindowForInternalVehicle))
					|| ((Settings.listRouteSourceDestinationWindowForInternalVehicle.size() > 0)
					&& roadNetwork.isNodeInsideRectangle(edge.endNode,
					Settings.listRouteSourceDestinationWindowForInternalVehicle))) {
	...
	void identifyReferencesOfAllPublicTransportTypesInSourceDestinationWindow() {
		identifyReferencesOfOnePublicTransportTypeInSourceDestinationWindow(internalTramRefInSdWindow, roadNetwork.tramRoutes,
		...
		identifyReferencesOfOnePublicTransportTypeInSourceDestinationWindow(internalBusRefInSourceDestinationWindow,
                roadNetwork.busRoutes, internalBusStartEdges, internalBusEndEdges, internalBusStartEdgesInSourceWindow,
	...
	public void parkOneVehicle(final Vehicle vehicle, final boolean isNewVehicle, final double timeNow) {
		...
		Edge edge = vehicle.routeLegs.get(vehicle.indexLegOnRoute).edge;
		edge.parkedVehicles.add(vehicle);
		if (isNewVehicle) {
    ...
    /**TO DO
     * HERE PROBABLY NEED TO USE THIS COOPY IN PSPBOARD/NONBOARD etc....
     */
	public void resetTraffic() {
        ...
        //workarea per worker should be enough -> in addition on Prometheus we use only one simulation so this is not very important to clear everything
		for (final Lane lane : roadNetwork.lanes) {
		    lane.vehicles.clear();
