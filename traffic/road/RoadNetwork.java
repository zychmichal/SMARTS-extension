...
//		Settings.numGridCols = Settings.numWorkers;
		Settings.numGridRows = Math.min(Settings.numWorkers, 720);
		Settings.numGridCols = Math.min(Settings.numWorkers, 720);
	...
	public int generateHash() {
		...
		for (int i = 0; i < nodes.size(); i++) {
			Node e = nodes.get(i);
			hashCode = 31 * hashCode + (e == null ? 0 : i*e.index*(int)e.osmId);
		}

		for (int i = 0; i < edges.size(); i++) {
			Edge e = edges.get(i);
			hashCode = 31 * hashCode + (e == null ? 0 : i*e.index*(int)e.length);
		}

		for (int i = 0; i < lanes.size(); i++) {
			Lane e = lanes.get(i);
			hashCode = 31 * hashCode + (e == null ? 0 : i*e.index);
		}
